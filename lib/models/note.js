...
module.exports = function (sequelize, DataTypes) {
  var Note = sequelize.define('Note', {
    ...
    category: {
      type: DataTypes.TEXT('long'),
      get: function () {
        var cat = this.getDataValue('category')
        if (cat == null) {
          return ''
        } else {
          return cat }
  ...
  Note.parseNoteInfo = function (body) {
    ...
    return {
      ...
      category: Note.extractNoteCategory(parsed.meta, $)
      // TODO: Category
    }
  }
  ...
  Note.parseNoteTags = function (body) {
    var parsed = Note.extractMeta(body)
    var $ = cheerio.load(md.render(parsed.markdown))
    return Note.extractNoteTags(parsed.meta, $)
  }
  ...
  Note.parseNoteCategory = function (body) {
    var parsed = Note.extractMeta(body)
    var $ = cheerio.load(md.render(parsed.markdown))
    return Note.extractNoteCategory(parsed.meta, $)
  }

  Note.extractNoteCategory = function (meta, $) {
    // TODO: we need this do get the category
    var tags = []
    var rawtags = []
    if (meta.tags && (typeof meta.tags === 'string' || typeof meta.tags === 'number')) {
      var metaTags = ('' + meta.tags).split(',')
      ...
      var h6s = $('h6')
      h6s.each(function (key, value) {
        if (/^category/gmi.test($(value).text())) {
          var codes = $(value).find('code')
          for (let i = 0; i < codes.length; i++) {
            var text = stripTags($(codes[i]).text().trim())
            if (text) rawtags.push(text)
