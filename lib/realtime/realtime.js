...
// const noteInfo = require('../models/note')
...
// TODO: this is the real shit!!!!! yes baby!
function buildNoteUpdateData (note) {
  const body = note.server.document
  const title = note.title = models.Note.parseNoteTitle(body)
  const category = note.category = models.Note.parseNoteCategory(body)
  //const tags = note.category = models.Note.parseNoteTags(body)
  return {
    title: title,
    content: body,
    category: category,
...
async function _updateNoteAsync (note) {
  ...
  //noteModel.category = 'test'
  if (!noteModel) return null

  updateHistoryForEveryUserCollaborateNote(note)
  // console.log(noteInfo)
  // var info = noteInfo.Note.extractNoteTags(noteModel.dataValues.content)
  // console.log(info)
  try {
